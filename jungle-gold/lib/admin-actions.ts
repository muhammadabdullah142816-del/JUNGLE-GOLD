...
import type { Product, CreateProductPayload, Operator, CreateOperatorPayload, OrderStatus } from "@/types/database";

// Create server-side Supabase client using Service Role Key if available, or Anon Key
...
// Re-verify admin session cookie on the server for all mutations
...
export async function uploadOperatorImageAction(formData: FormData): Promise<string> {
  ...
  const fileExt = file.name.split(".").pop();
  const fileName = `operator-${Date.now()}-${Math.random().toString(36).substring(7)}.${fileExt}`;
  ...
  if (uploadError) throw new Error("Failed to upload operator image: " + uploadError.message);
